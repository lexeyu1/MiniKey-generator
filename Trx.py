...
for i in range(0, len(mylist)):
    ...
    if float(bal) > 0:
        win += 1
        f = open("Found_TRX.txt", "a")
        f.write('\nAddresss: ' + str(addr) + ' BALANCE: ' + str(bal))
        f.write('\nPRIVATEKEY: ' + str(priv))
        f.write('\n---------------------------------------------------------------\n')
        print(str(count), str(addr), 'BALANCE:', str(bal),      'PASSPHRASE: ', str(passphrase))
    else:
        print(str(count), 'WIN:', str(win), str(addr), 'BALANCE:', str(bal),        'PASSPHRASE: ', str(passphrase))
